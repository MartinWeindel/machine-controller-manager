...
func (r *Subresource) SaveDevIDs(device types.BaseVirtualDevice, ctlr types.BaseVirtualController) error {
	r.data["key"] = fmt.Sprintf("%s", device.GetVirtualDevice().Key)
	addr, err := computeDevAddr(device, ctlr)
	if err != nil {
		return err
	}
	r.data["device_address"] = addr
	return nil
}
...
func (r *Subresource) DevAddr() string {
	return r.GetDataString("device_address")
}
...
func (r *Subresource) FindVirtualDeviceByAddr(l object.VirtualDeviceList) (types.BaseVirtualDevice, error) {
	glog.V(4).Infof("[DEBUG] FindVirtualDevice: Looking for device with address %s", r.DevAddr())
	...
	device := devices[0]
	glog.V(4).Infof("[DEBUG] FindVirtualDevice: Device found: %s", l.Name(device))
	return device, nil
}

func (r *Subresource) GetDataInt(key string) int {
	value, ok := r.data[key]
	if !ok {
		return 0
	}
...
func (r *Subresource) GetDataString(key string) string {
	value, ok := r.data[key]
	if !ok {
		return ""
	}
...
func (r *Subresource) GetDataBoolWithVeto(key string) (bool, error) {
	/* TODO martin change detection how, needed ???
	if r.HasChange(key) {
...
func (r *Subresource) FindVirtualDevice(l object.VirtualDeviceList) (types.BaseVirtualDevice, error) {
	if key := r.GetDataInt("key"); key > 0 {
		glog.V(4).Infof("[DEBUG] FindVirtualDevice: Looking for device with key %d", key)
		if dev := l.FindByKey(int32(key)); dev != nil {
			glog.V(4).Infof("[DEBUG] FindVirtualDevice: Device found: %s", l.Name(dev))
			return dev, nil
		}
...
func swapSCSIDevice(l object.VirtualDeviceList, device types.BaseVirtualSCSIController, ct string, st string) ([]types.BaseVirtualDeviceConfigSpec, error) {
	glog.V(4).Infof("[DEBUG] swapSCSIDevice: Swapping SCSI device for one of controller type %s: %s", ct, l.Name(device.(types.BaseVirtualDevice)))
	...
	glog.V(4).Infof("[DEBUG] swapSCSIDevice: Outgoing device config spec: %s", DeviceChangeString(spec))
	return spec, nil
}
...
func NormalizeSCSIBus(l object.VirtualDeviceList, ct string, count int, st string) (object.VirtualDeviceList, []types.BaseVirtualDeviceConfigSpec, error) {
	glog.V(4).Infof("[DEBUG] NormalizeSCSIBus: Normalizing first %d controllers on SCSI bus to device type %s", count, ct)
	var spec []types.BaseVirtualDeviceConfigSpec
	...
	glog.V(4).Infof("[DEBUG] NormalizeSCSIBus: Current SCSI bus contents: %s", scsiControllerListString(ctlrs))
	// Now iterate over the controllers
	for n, ctlr := range ctlrs {
		if ctlr == nil {
			glog.V(4).Infof("[DEBUG] NormalizeSCSIBus: Creating SCSI controller of type %s at bus number %d", ct, n)
	...
	glog.V(4).Infof("[DEBUG] NormalizeSCSIBus: Outgoing device list: %s", DeviceListString(l))
	glog.V(4).Infof("[DEBUG] NormalizeSCSIBus: Outgoing device config spec: %s", DeviceChangeString(spec))
	return l, spec, nil
}
...
func pickSCSIController(l object.VirtualDeviceList, bus int) (types.BaseVirtualController, error) {
	glog.V(4).Infof("[DEBUG] pickSCSIController: Looking for SCSI controller at bus number %d", bus)
	...
	glog.V(4).Infof("[DEBUG] pickSCSIController: Found SCSI controller: %s", l.Name(l[0]))
...
func (r *Subresource) ControllerForCreateUpdate(l object.VirtualDeviceList, ct string, bus int) (types.BaseVirtualController, error) {
	glog.V(4).Infof("[DEBUG] ControllerForCreateUpdate: Looking for controller type %s", ct)
	...
	glog.V(4).Infof("[DEBUG] ControllerForCreateUpdate: Found controller: %s", l.Name(ctlr.(types.BaseVirtualDevice)))
...
func applyDeviceChange(l object.VirtualDeviceList, cs []types.BaseVirtualDeviceConfigSpec) object.VirtualDeviceList {
	glog.V(4).Infof("[DEBUG] applyDeviceChange: Applying changes: %s", DeviceChangeString(cs))
	glog.V(4).Infof("[DEBUG] applyDeviceChange: Device list before changes: %s", DeviceListString(l))
	for _, s := range cs {
	...
	glog.V(4).Infof("[DEBUG] applyDeviceChange: Device list after changes: %s", DeviceListString(l))
	return l
}

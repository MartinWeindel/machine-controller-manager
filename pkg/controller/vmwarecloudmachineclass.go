...
func (c *controller) reconcileClusterVmwareMachineClassKey(key string) error {
	...
	if errors.IsNotFound(err) {
		glog.Infof("%s %q: Not doing work because it has been deleted", VsphereMachineClassKind, key)
		return nil
	}
	if err != nil {
		glog.Infof("%s %q: Unable to retrieve object from store: %v", VsphereMachineClassKind, key, err)
		return err
	}
...
func (c *controller) reconcileClusterVsphereMachineClass(class *v1alpha1.VsphereMachineClass) error {
	glog.V(4).Info("Start Reconciling vspheremachineclass: ", class.Name)
	defer func() {
		c.enqueueVsphereMachineClassAfter(class, 10*time.Minute)
		glog.V(4).Info("Stop Reconciling vspheremachineclass: ", class.Name)
	}()
	...
	if validationerr.ToAggregate() != nil && len(validationerr.ToAggregate().Errors()) > 0 {
		glog.V(2).Infof("Validation of %s failed %s", VsphereMachineClassKind, validationerr.ToAggregate().Error())
		return nil
	}
...
func (c *controller) updateVsphereMachineClassFinalizers(class *v1alpha1.VsphereMachineClass, finalizers []string) error {
	...
	if err != nil {
		glog.Warning("Updating VsphereMachineClasses failed, retrying. ", class.Name, err)
		return err
	}
	glog.V(3).Infof("Successfully added/removed finalizer on the vspheremachineclass %q", class.Name)
	return err
}
...
func (c *controller) enqueueVsphereMachineClassAfter(obj interface{}, after time.Duration) {
	...
	c.vsphereMachineClassQueue.AddAfter(key, after)
}
